...
IdAttaque["Attaque Précise"]="IdComp9";
IdAttaque["Botte Secrète"]="IdComp1";
IdAttaque["Frénésie"]="IdComp7";
...
function catchPiege() {     
  if (document.URL.indexOf("Play_a_Competence15b.php")>=0) {  //Un piege à été posé
      var nodes = document.evaluate("//b/text()[contains(.,'Vous avez posé un Piège')]", document, null,XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
      for (var i = 0; i < nodes.snapshotLength; i++) {
        var Piege = nodes.snapshotItem(i).nodeValue;
        var TypePiege = Piege.slice(Piege.indexOf("Piège à")+8, Piege.indexOf("en")-1);        
    ...
    if(document.referrer.indexOf("Play_a_Sort10.php")>=0) {
		...
		if (nodes.snapshotLength>0) {
	        ...
	        for (var i = 0; i < nodes.snapshotLength; i++) {
				...
				matosData = matosData+'&Etat[]='+URLencode('[Identifié au sol]');				      
...
function catchAttaque() {
	...
	{
		if (document.URL.indexOf("Play_a_SortResult.php")>=0) {	// cas d'une xplo, on vérifie que s'en est bien une
...
function catchBOTDegatPiege() {     
	if (document.URL.indexOf("Messagerie/ViewMessageBot.php")>=0) {
		...
		if (messageTitle.indexOf("[MountyHall] Déclenchement de Piège") != -1) {
			var td = document.evaluate("//td/text()[contains(.,'a déclenché votre Piège à Feu')]/..", document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
			if (!td) return "";
			...
			TimeStamp=TimeStamp.substring(0,TimeStamp.indexOf(".")).replace(" ", "_");
	  		...
	  		data+="&TimeStamp="+TimeStamp;
	  		data+="&TiD="+numTroll;
	  		data+="&Attaque=IdComp15&REG=0&NIV=0";		//Compétence15=piege
	  		data+=DataAttaque;
...
function catchBOTIdT2() {     
	...
	if (document.URL.indexOf("Messagerie/ViewMessageBot.php")>=0) {
		...
		if (messageTitle.indexOf("[MountyHall] Sortilège : Identification des trésors") != -1) {
			...
			if (IdT.indexOf("Le trésor se trouve à vos pieds")>0) matosEtat='[Identifié au sol]';
...
function catchBOTInsulte() {     
	...
	if (document.URL.indexOf("Messagerie/ViewMessageBot.php")>=0) {
		...
		if (messageTitle.indexOf("[MountyHall] Compétence Insulte") != -1) {
			...
			var sr=Insulte.substring(Insulte.indexOf("Seuil de Résistance de la Cible"));
...
function catchBOTAttaque() {     
	if (document.URL.indexOf("Messagerie/ViewMessageBot.php")>=0) {
		...
		if ((messageTitle.indexOf("[MountyHall] Résultat de Combat - Attaquant") != -1)||
		    ...
		    if (messageTitle.indexOf("[MountyHall] Sortilège : Explosion") != -1) var ATTAQUE="IdSort8";  else {
	  			nodes = document.evaluate("//td/text()[contains(.,'grâce à une compétence')]", document, null,XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
  				if (nodes.snapshotLength>0) var ATTAQUE="IdComp"; else var ATTAQUE="IdSort";  
			}

			// recherche du niveau de la Cible => impossible sur le BOT, le troll à put changer de niveau
			//...
...
function catchVL() {
	if (document.URL.indexOf("Play_a_SortResult.php")>=0) 
	{
	  ...
	  var myB=document.createElement('i');
	  myB.appendChild(document.createTextNode("Patientez, la page est en cours d'envoi à l'outil ZZ!"));
	  myB.setAttribute("class", "titre5");
...
// Function getLevel() has been removed from MZ original code for ZZ Fusion
...
function dispatch() {
	if (isPage("MH_Play/Actions")) {
		sendDices();
		if(document.evaluate("//tr/td/descendant::p/text()[contains(., 'Zone centrale ciblée')]",document, null, XPathResult.STRING_TYPE, null).stringValue) return; else if (document.evaluate("//form/descendant::p/text()[contains(., 'Vous êtes entré dans une Zone Piégée')]",
			document, null, XPathResult.STRING_TYPE, null).stringValue)
			traiteMM();
			...
			document, null, XPathResult.STRING_TYPE, null).stringValue)
		{
			...
			traiteRM();
			// getLevel(); //ZZ: remplacé car ne fonctionne pas correctement et ZZ le calcul ailleurs
		}
	} else {
		var messageTitle = document.getElementsByTagName('table')[0].childNodes[1].childNodes[0].childNodes[1].childNodes[0].childNodes[1].firstChild.nodeValue;
