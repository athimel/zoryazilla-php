...
var portailInfo=new Array();		// Carac des Trésros de la coterie
...
var idPortails="";				// Liste des portails les plus proches
...
var nbTabSupTroll=0;			// Décalage du tableau des Troll par ajout des boutons MP, Vue, etc...
var isNaplumange=false;			// cas de MH sans monstre si pouvoir du fumeux Naplumangé ?
...
for (i=0; i<totaltab.length; i++) {
	...
	try {ttab=totaltab[i].childNodes[1].childNodes[0].childNodes[0].childNodes[1].childNodes[0].childNodes[0].childNodes[2].childNodes[0].childNodes[0].childNodes[0].nodeValue;} catch (e) {}			

	if (ttab=="MONSTRES ERRANTS") {var itmon=i; }
	if (ttab=="TRÕLLS") {var ittro=i; }
	if (ttab=="TRÉSORS"){var ittre=i; }		
	if (ttab=="CHAMPIGNONS") {var itcha=i; }	
	if (ttab=="LIEUX PARTICULIERS") {var itlie=i; }
...
// Différents tableaux			//FZZCHG
if (isNaplumange) {
	var totaltab = document.getElementsByTagName('table');
	var x_monstres = totaltab[6].getElementsByTagName('tr');
	var nbMonstres = 0 ;		//ZZCHG: pour concerver le codage actuelle
	var x_trolls = totaltab[6].getElementsByTagName('tr');
	var nbTrolls = x_trolls.length ;			//ZZCHG: pour concerver le codage actuelle
	var x_tresors = totaltab[10].getElementsByTagName('tr');
	var nbTresors = x_tresors.length ;		//ZZCHG: pour concerver le codage actuelle
	var x_champis = totaltab[14].getElementsByTagName('tr');
	var nbChampis = x_champis.length ;		//ZZCHG: pour concerver le codage actuelle
	var x_lieux = totaltab[18].getElementsByTagName('tr');
	var nbLieux = x_lieux.length ;			//ZZCHG: pour concerver le codage actuelle
} else {
	var totaltab = document.getElementsByTagName('table');
	var x_monstres = totaltab[6].getElementsByTagName('tr');
	var nbMonstres = x_monstres.length ;		//ZZCHG: pour concerver le codage actuelle
	var x_trolls = totaltab[10].getElementsByTagName('tr');
	var nbTrolls = x_trolls.length ;			//ZZCHG: pour concerver le codage actuelle
	var x_tresors = totaltab[14].getElementsByTagName('tr');
	var nbTresors = x_tresors.length ;		//ZZCHG: pour concerver le codage actuelle
	var x_champis = totaltab[18].getElementsByTagName('tr');
	var nbChampis = x_champis.length ;		//ZZCHG: pour concerver le codage actuelle
	var x_lieux = totaltab[22].getElementsByTagName('tr');
	var nbLieux = x_lieux.length ;			//ZZCHG: pour concerver le codage actuelle
}
...
function filtreMonstres() {
	if (isNaplumange) return;		// pas de monstre (pouvoir du fumeux)
	...
	totaltab[4].childNodes[1].firstChild.firstChild.childNodes[1].firstChild.firstChild.childNodes[2].firstChild.firstChild.firstChild.	//ZZCHG [.1.0.0.1.0.0.2.0.0.0
				nodeValue = "MONSTRES ERRANTS" + (filtre ? " (filtrés sur " + filtreMonstre + ")" : "");
	if(niveau_min>0 || niveau_max>0)
	{
		var node = totaltab[4].childNodes[1].firstChild.firstChild.childNodes[1].firstChild.firstChild.childNodes[2].firstChild.firstChild.firstChild;	//ZZCHG
	...
	for (var i = nbMonstres; --i >= 2;) {		//FZZCHG
...
function filtreLevels() {
	...
	if (!isCDMsRetrieved)
		return;
	//x_monstres[2].removeChild(x_monstres[2].childNodes[2]);		//FZZCHG
	for (var i = 1; i < nbMonstres+1; i++) {						//FZZCHG
...
function retrieveCDMs() {
	if (checkBoxLevels.checked)
		return;
	var str = "";
	var begin = 2;							//FZZCHG 3=>2
	var max = MZ_getValue("MAX_LEVEL");
	if(MZ_getValue('CDMID')==null)
		MZ_setValue('CDMID',1);
	max = Math.min(nbMonstres+1, (max == "" || max== null) ? 5000 : max)-1;			//FZZCHG -1
	for (var i = 2; i < max; i++) {			//FZZCHG 3=>2
		...
		if (i % 2000 == 0 || i == max - 1)
		{	
			//alert(ZZDB+'/mzMonstres.php?begin=' + begin+'&idcdm=' + MZ_getValue('CDMID') + '&' + str);
...
function insertLevelColumn() {
	var tr = insertTdText(getMonstreLevelNode(1), 'Niveau', true);				//FZZCHG
...
function filtreTrolls() {
	...
	totaltab[8].childNodes[1].firstChild.firstChild.childNodes[1].firstChild.firstChild.childNodes[2].firstChild.firstChild.firstChild
		.nodeValue = "TRÕLLS" + (filtreT ? " (filtrés sur " + filtreTroll + ")" : "") + (filtreG ? " (guildes filtrées sur " + filtreGuilde + ")" : "");

	for (var i = nbTrolls; --i >= 1;) {		//FZZCHG
		var tds = x_trolls[i].childNodes;
		x_trolls[i].style.display = (noIntangibles && tds[2+shift].firstChild.className == 'mh_trolls_0')
				|| (filtreT && tds[2+shift].firstChild.firstChild.nodeValue.toLowerCase().indexOf(filtreTroll) == -1)
				|| (filtreG && (!tds[5+shift].firstChild.firstChild || tds[5+shift].firstChild.firstChild.nodeValue.toLowerCase().indexOf(filtreGuilde) == -1))
				? 'none' : '';
	}
}
...
function filtreTresors() {
	...
	if (noEngages && !isEngagesComputed) {			//FZZCHG
		for (var i = nbTrolls; --i >= 2;) 
		{	
			var pos = getTrollPosition(i);
			if (!listeEngages[pos[0]])
	...
	totaltab[nbTabSup+13].firstChild.firstChild.childNodes[2].firstChild.firstChild.firstChild.nodeValue = "TRESORS" + (filtre ? " (filtrés sur " + filtreTresor + ")" : "");
				
	for (var i = nbTresors; --i >= 2;) {			//FZZCHG
...
function filtreLieux() {
	...
	totaltab[nbTabSup+21].firstChild.firstChild.childNodes[2].firstChild.firstChild.firstChild.
			nodeValue = "LIEUX PARTICULIERS" + (filtre ? " (filtrés sur " + filtreLieu + ")" : "");
	for (var i = nbLieux; --i >= 2;)	{	//FZZCHG  [.0.0.2.0.0.0=LIEUX]
...
function putLieuxBouton() {
	insertBouton(totaltab[20], 'http://mountyzilla.tilk.info/scripts/lieux.php',			//FZZCHG
...
{
	for (var i = 2; i < nbTrolls; i++) 	//FZZCHG
	{
...
function creerTHead(num) {
	return;		// FZZCHG: a voir plus tard, plus de table pliable pour le moment
...
start_script(31);
	...
	creerTHead(4);
//creerTHead(3);					// FZZHG (boite pliable Infos)
//for (var i = 4; i <= 24; i += 4)	// FZZHG
//	creerTHead(i);
...
function analyseCDM (idx) {
   ...
   var cdmMonstre = listeCDM[id];  

   if (x_monstres[2+idx].childNodes[2].childNodes[0].nodeValue=="-") return;		// pas de CdM possible
   if ((fullname.indexOf("Gowap")>=0) &&(cdmMonstre[11]!='???')) return;		// Gowap sans CdM

   var Bestiaire="BESTIAIRE"; 
   var barrePV;
   var Blessure;
   if (cdmMonstre[11].indexOf("%")>0) {

    	Bestiaire="CDM du <br><b>"+cdmMonstre[11].slice(cdmMonstre[11].indexOf('le')+2)+"</b>";
		Blessure="&nbsp&nbsp&nbsp"+bbcode(cdmMonstre[11]); 
		if (cdmMonstre[11].indexOf("%")!=-1) {
			var b=1*cdmMonstre[11].slice(cdmMonstre[11].indexOf("[b]")+3,  cdmMonstre[11].indexOf(" %"));
			if ((b!=0) && (cdmMonstre[2].indexOf("-")!=-1)) {
		        var pv2 = cdmMonstre[2].substring(0,cdmMonstre[2].indexOf("-"))*1;
		        var pv1 = cdmMonstre[2].substring(cdmMonstre[2].indexOf("-")+1,cdmMonstre[2].indexOf(" -->"))*1;
		    ...
		    } else if ((b!=0) && (cdmMonstre[2].indexOf("[b]")!=-1)) {
				var pv1=1*cdmMonstre[2].slice(cdmMonstre[2].indexOf("[b]")+3, cdmMonstre[2].indexOf("[/b]"));
		        var p1=92.5;  if (b!=95) p1=b-5; if (p1<0) p1=0;
		        var pva1 = Math.floor(pv1 * (100 - p1) / 100);
		        var p2=92.5;  if (b!=90) p2=b+5; if (p2>100) p2=100;
    	...
    	var pv=100-cdmMonstre[11].substring(cdmMonstre[11].indexOf(']')+1,cdmMonstre[11].indexOf('%'));
    	var barrePV=createBarrePV(0, pv, 100, ''); 
   }
   
   // information de SR
   var cdmRM=bbcode(cdmMonstre[9]);
   if((cdmMonstre[9].indexOf("[b]")!=-1) && (MM_TROLL!="")) {
		var v=0;
		var mrm=1*cdmMonstre[9].slice(cdmMonstre[9].indexOf("[b]")+3,  cdmMonstre[9].indexOf("[/b]"));
		var tmm=MM_TROLL*1;
		if(tmm<0) v="10";
   ...
   var cdmMM="";
   // information de SR
   var cdmMM=cdmMonstre[18];
   if((cdmMonstre[18].indexOf("[b]")!=-1) && (RM_TROLL!="")) {
		var v=0;
		var mmm=1*cdmMonstre[12].slice(cdmMonstre[12].indexOf("[b]")+3,  cdmMonstre[12].indexOf("[/b]"));
		var trm=RM_TROLL*1;
		if(trm<0) v="90";
   ...
   if (idMonstre>=0) var photo=SkinZZ+'/monstres/'+skinBEAST[tabMonstres[idMonstre][3]]; else photo=SkinZZ+'/monstres/'+skinBEAST[0];
	...
	if (cdmMonstre[12]==1) vlc="oui"; else if (cdmMonstre[12]==0) vlc="non"; else vlc="";
	if(cdmMonstre[13]==1) attdist="oui"; else if(cdmMonstre[13]==0) attdist="non"; else attdist="";
	//if(cdmMonstre[14]==1) att=1; else if(cdmMonstre[14]>1 && cdmMonstre[14]<=6) att=cdmMonstre[14]; else if(cdmMonstre[14]>6) attdist="Beaucoup";
	if(cdmMonstre[14]) att=cdmMonstre[14];
	if(cdmMonstre[15]!="") vit=bbcode(cdmMonstre[15]); 
	if(cdmMonstre[17] && cdmMonstre[17].length>0) equip=bbcode(cdmMonstre[17]);
	if(cdmMonstre[16]) range=bbcode(cdmMonstre[16]);
	if(cdmMonstre[10]) pouvoir=bbcode(cdmMonstre[10]);
	if(cdmMonstre[19]) dla=bbcode(cdmMonstre[19]);
	if(cdmMonstre[20]) BM=bbcode(cdmMonstre[20]);
	if(cdmMonstre[21]) tour=bbcode(cdmMonstre[21]);
	if(cdmMonstre[22]) AM=bbcode(cdmMonstre[22]);
	if(cdmMonstre[23]) { if (cdmMonstre[23]==1) vole="oui"; else if (cdmMonstre[23]==0) vole="non"; else vole=""; }
	if(cdmMonstre[24]) SF=bbcode(cdmMonstre[24]);
	if(cdmMonstre[25]) { if (cdmMonstre[25]==1) attmag="oui"; else if (cdmMonstre[25]==0) attmag="non"; else attmag=""; } 
	
   var strCDM ='<TABLE class="mh_tdborder" width="690" BORDER="0" CELLSPACING="1" CELLPADDING="2">';
   strCDM+='<TR class="mh_tdtitre"><TD>NIV: '+bbcode(cdmMonstre[0]) + analysePX(bbcode(cdmMonstre[0]))+'</td><TD colspan="4">'+id+' '+fullname+'</TD><TD colspan="2" align="center">'+bbcode(cdmMonstre[1])+'</TD></TR>';
   strCDM+='<TR class="mh_tdpage"><TD rowspan="8" align="center" valign="center"><IMG SRC="'+photo+'"></td><TD class="mh_tdtitre">PdV:</TD><TD>'+bbcode(cdmMonstre[2])+'</TD><TD colspan="4">';
   if (cdmMonstre[11].indexOf("%")>0) strCDM+=barrePV.innerHTML+" "+Blessure; strCDM+='</TD></TR>';
   strCDM+='<TR class="mh_tdpage"><TD class="mh_tdtitre">ATT:</TD><TD>'+bbcode(cdmMonstre[3])+'</TD><TD class="mh_tdtitre">VUE:</TD><TD>'+bbcode(cdmMonstre[8])+'</TD><TD colspan="2" rowspan="2" class="mh_tdtitre" align="center" valign="center">'+Bestiaire+'</TD></TR>';
   strCDM+='<TR class="mh_tdpage"><TD class="mh_tdtitre">ESQ:</TD><TD>'+bbcode(cdmMonstre[4])+'</TD><TD class="mh_tdtitre">VLC:</TD><TD>'+vlc+'</TD></TR>';
   strCDM+='<TR class="mh_tdpage"><TD class="mh_tdtitre">DEG:</TD><TD>'+bbcode(cdmMonstre[5])+'</TD><TD class="mh_tdtitre">M.M:</TD><TD>'+cdmMM+'</TD><TD class="mh_tdtitre">VIT:</TD><TD>'+vit+'</TD></TR>';
   strCDM+='<TR class="mh_tdpage"><TD class="mh_tdtitre">REG:</TD><TD>'+bbcode(cdmMonstre[6])+'</TD><TD class="mh_tdtitre">R.M:</TD><TD>'+cdmRM+'</TD><TD class="mh_tdtitre">TR.:</TD><TD>'+tour+'</TD></TR>';
   strCDM+='<TR class="mh_tdpage"><TD class="mh_tdtitre">ARM:</TD><TD>'+bbcode(cdmMonstre[7])+'</TD><TD class="mh_tdtitre">Att:</TD><TD>'+att+'</TD><TD class="mh_tdtitre">DLA:</TD><TD>'+dla+'</TD></TR>';
...
function sendVue(VueId) { 
	...
	if (flowVueCtrl=="Mon") {
 		while ((flowVueCtrlId<x_monstres.length) && (nrow<maxRow)) {
  		...
  		if (flowVueCtrlId>=x_monstres.length) { flowVueCtrl="Tro";  flowVueCtrlId=2; flowVueCtrlMaxId=x_trolls.length;} 
	}
	...
	if (flowVueCtrl=="Tro") {
		var shift = (incT == 0 ? 0 : 1);
 		while ((flowVueCtrlId<x_trolls.length) && (nrow<maxRow)) {
  		...
  		if (flowVueCtrlId>=x_trolls.length) { flowVueCtrl="Tre";  flowVueCtrlId=2; flowVueCtrlMaxId=x_tresors.length;} 
 	}

	var txtTre="";
	if (flowVueCtrl=="Tre") {	// trop de trésor, on prend les plus proches
 		while ((flowVueCtrlId<Math.min(3,x_tresors.length)) && (nrow<maxRow)) {
  		...
  		if (flowVueCtrlId>=x_tresors.length) { flowVueCtrl="Cha";  flowVueCtrlId=2; flowVueCtrlMaxId=x_champis.length;} 
 	}
	...
	if (flowVueCtrl=="Cha") {
 		while ((flowVueCtrlId<x_champis.length) && (nrow<maxRow)) {
  		...
  		if (flowVueCtrlId>=x_champis.length) { flowVueCtrl="Lie";  flowVueCtrlId=2; flowVueCtrlMaxId=x_lieux.length;} 
 	}
	...
	if (flowVueCtrl=="Lie") {
 		...
 		if (flowVueCtrlId>=x_lieux.length) { flowVueCtrl="";  flowVueCtrlId=2; flow="END"; } 
 	}
...
function putInfosVUE() { 
  ...
  if (!isNaplumange) setInfosMonstres(2) ; 	
