...
// Différents tableaux
var index_monstres = 7;
var index_trolls = 11;
var index_tresors = 15;
var index_champis = 19;
var index_lieux = 23;
var index_debut_table = 1;
var totaltab = document.getElementsByTagName('table');
var x_monstres = totaltab[index_monstres].getElementsByTagName('tr');
var nbMonstres = x_monstres.length - 1;
var x_trolls = totaltab[index_trolls].getElementsByTagName('tr');
var nbTrolls = x_trolls.length - 1;
var x_tresors = totaltab[index_tresors].getElementsByTagName('tr');
var nbTresors = x_tresors.length - 1;
var x_champis = totaltab[index_champis].getElementsByTagName('tr');
var nbChampis = x_champis.length - 1;
var x_lieux = totaltab[index_lieux].getElementsByTagName('tr');
...
function filtreMonstres() {
	...
	if (noEngages && !isEngagesComputed) {
		for (var i = nbTrolls+1; --i >= index_debut_table;) {
	...
	totaltab[index_monstres-2].rows[0].cells[1].firstChild.firstChild.innerHTML= "MONSTRES ERRANTS" + (filtre ? " (filtrés sur " + filtreMonstre + ") " : " ") + strfilter;
	for (var i = nbMonstres+1; --i >= index_debut_table;) {
...
function filtreLevels() {
	...
	if (!isCDMsRetrieved)
		return;
	for (var i = index_debut_table; i < nbMonstres+2; i++) {
...
function removeLevelColumn() {
	if (totaltab[index_monstres].rows[0].cells[2].firstChild.innerHTML == 'Niveau')
        totaltab[index_monstres].rows[0].removeChild(totaltab[index_monstres].rows[0].cells[2]);
}

function insertLevelColumn() {
	var tr = insertTdText(getMonstreLevelNode(index_debut_table-1), 'Niveau', true);
...
function filtreTrolls() {
	...
	totaltab[index_trolls-2].rows[0].cells[1].firstChild.firstChild.innerHTML= "TROLLS" + (filtreT ? " (filtrés sur " + filtreTroll + ")" : "")
...
function filtreTresors() {
	...
	totaltab[nbTabSup+index_tresors-2].rows[0].cells[1].firstChild.firstChild.innerHTML = "TRESORS" + (filtre ? " (filtrés sur " + filtreTresor + ")" : "");
	for (var i = nbTresors+1; --i >= index_debut_table;) {
...
function filtreLieux() {
	...
	totaltab[nbTabSup+index_lieux-2].rows[0].cells[1].firstChild.firstChild.innerHTML = "LIEUX PARTICULIERS" + (filtre ? " (filtrés sur " + filtreLieu + ")" : "");
	for (var i = nbLieux+1; --i >= index_debut_table;)
...
function putMonstresBouton() {
	insertBouton (totaltab[index_monstres-3], 'http://mountyhall.clubs.resel.fr/script/v2/get_monstres.php',
			'listemonstres', getMonstres, 'Ajouter les monstres à la base des Teubreux');
}

function putLieuxBouton() {
	insertBouton(totaltab[index_lieux-3], 'http://mountyzilla.tilk.info/scripts/lieux.php',
...
function appendMonstres(txt) {
	for (var i = index_debut_table; i <= nbMonstres; i++)
...
function appendLieux(txt) {
	for (var i = index_debut_table; i < nbLieux+1; i++) {
...
{
	var i;
	for (i = index_debut_table; i < nbTrolls+1; i++)
		if (infosTrolls[getTrollID(i)])
			break;
	if (i == nbTrolls+1)
		return;

	var td = insertTdText(x_trolls[0].childNodes[6], 'PV', true);
	td.setAttribute('width', '105');
	td = insertTdText(x_trolls[0].childNodes[7], 'PA', true);
