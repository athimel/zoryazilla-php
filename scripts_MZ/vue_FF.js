...
totaltab = document.getElementsByClassName('mh_tdborder');
var itbid = -1; //Bidiview pas toujours présente
for (i = 0; i < totaltab.length; i++) {
    var ttab = "";
    var ttest = "";
    try {
       //ttab = totaltab[i].childNodes[0].childNodes[0].childNodes[0].childNodes[0].childNodes[0].childNodes[0].nodeValue;
	   ttab = totaltab[i].childNodes[1].childNodes[0].childNodes[0].childNodes[1].childNodes[0].childNodes[0].childNodes[2].childNodes[0].childNodes[0].childNodes[0].nodeValue;
    } catch (e) {
}
    try {
        //ttest = totaltab[i].childNodes[0].childNodes[0].childNodes[2].childNodes[0].childNodes[0].childNodes[0].nodeValue;
    } catch (e) {}
    if (ttab == "LA VUE") {
        externalVue = true;
    }
    if ((ttab == "MA VUE") || (ttab == "LA VUE")) {
        var itinf = i + 1;
    }
    if (ttab == "MONSTRES ERRANTS" || ttest == "MONSTRES ERRANTS") {
        var itmon = i - 1;
    }
    if (ttab == "TRÕLLS" || ttest == "TRÕLLS") {
        var ittro = i - 1;
    }
    if (ttab == "TRÉSORS" || ttest == "TRÉSORS") {
        var ittre = i - 1;
    }
    if (ttab == "CHAMPIGNONS" || ttest == "CHAMPIGNONS") {
        var itcha = i - 1;
    }
    if (ttab == "LIEUX" || ttest == "LIEUX") {
        var itlie = i - 1;
    }
    try {
        ttab = totaltab[i].childNodes[0].childNodes[0].childNodes[0].childNodes[0].childNodes[0].nodeValue;
    } catch (e) {}
    if (ttab == "B I D I V I E W") {
        itbid = i;
    }
}
...
//alert('tinf:'+itinf+'   tmon:'+itmon+'  ttro:'+ittro+'   ttre:'+ittre+'   tcha:'+itcha+'   tlie:'+itlie);
...
// Change mainTabs[2] to mainTabs[mainTabs.length - 11] in case a div is added before MONSTRES.
// Normal length is 13

var monstresIndex = mainTabs.length - 11;
//var x_monstres = mainTabs[2].getElementsByTagName('tr');
var x_monstres = mainTabs[monstresIndex].getElementsByTagName('tr');
var nbMonstres = x_monstres.length - 1;
//var x_trolls = mainTabs[4].getElementsByTagName('tr');
var x_trolls = mainTabs[mainTabs.length - 9].getElementsByTagName('tr');
var nbTrolls = x_trolls.length - 1;
//var x_tresors = mainTabs[6].getElementsByTagName('tr');
var x_tresors = mainTabs[mainTabs.length - 7].getElementsByTagName('tr');
var nbTresors = x_tresors.length - 1;
//var x_champis = mainTabs[8].getElementsByTagName('tr');
var x_champis = mainTabs[mainTabs.length - 5].getElementsByTagName('tr');
var nbChampis = x_champis.length - 1;
//var x_lieux = mainTabs[10].getElementsByTagName('tr');
var x_lieux = mainTabs[mainTabs.length - 3].getElementsByTagName('tr');
var nbLieux = x_lieux.length - 1;

//console.log("nbMonstres: " + nbMonstres);
//console.log("nbTrolls: " + nbTrolls);
...
function saveCheckBoxStatus(chB, pref) {
    var etat = chB.checked;
    MZ_setValue(pref, etat ? 'true' : 'false');
    return etat;
}

function recallCheckBoxStatus(chB, pref) {
    chB.checked = (MZ_getValue(pref) == 'true');
}
...
function filtreMonstres() {
    ...
    mainTabs[mainTabs.length - 12].rows[0].cells[0].childNodes[1].rows[0].cells[1].firstChild.firstChild.innerHTML =
...
function filtreLevels() {
    ...
    for (var i = nbMonstres; i > 0; i--) {
        listeLevels[i] = getMonstreLevelNode(i).innerHTML; // mémorisation ? Dab
...
function removeLevelColumn() {
    if (mainTabs[monstresIndex].rows[0].cells[2].firstChild.innerHTML == 'Niveau')
        mainTabs[monstresIndex].removeChild(mainTabs[monstresIndex].rows[0].cells[2]);
}

function insertLevelColumn() {
    var tr = insertTdText(getMonstreLevelNode(1 - 1), 'Niveau', true);
...
function getTrollPosition(i) {
    ...
    var j = tds.length;
    return [tds[j - 3].firstChild.nodeValue, tds[j - 2].firstChild.nodeValue, tds[j - 1].firstChild.nodeValue];
}
...
function filtreTrolls() {
    ...
    mainTabs[mainTabs.length - 10].rows[0].cells[0].childNodes[1].rows[0].cells[1].firstChild.firstChild.innerHTML =
...
function filtreTresors() {
    ...
    mainTabs[mainTabs.length - 8].rows[0].cells[0].childNodes[1].rows[0].cells[1].firstChild.firstChild.innerHTML =
...
function filtreLieux() {
    ...
    mainTabs[mainTabs.length - 4].rows[0].cells[0].childNodes[1].rows[0].cells[1].firstChild.firstChild.innerHTML =
...
function putExternalLinks() { // liens intégrables déclarés dans les options MZ
    var insDiv = document.evaluate("//div/a[contains(./text(),'Logout')]", document,
    ...
    if (cookie && cookie != '') {
        ...
        insertBefore(insDiv, myDiv);
        var a = document.createElement('A');
        myDiv.appendChild(a);
        a.setAttribute('href', cookie);
        a.setAttribute('target', '_blank');
        a.setAttribute('CLASS', 'AllLinks');
        appendText(a, '[' + MZ_getValue('NOM1') + ']');
...
function refresh2DViewButton() {
    ...
    function () {
        document.getElementsByName(vue2Ddata[vueext][1])[0].value = vue2Ddata[vueext][2]();
    });
}
...
function set2DViewSystem() { // choix de la vue sans passer par les options
    ...
    for (var view in vue2Ddata) {
        ...
        if (view == vueext) {
            selectVue2D.selectedIndex = ind;
        }
        ind++;
    }
    selectVue2D.addEventListener('mousemove', refresh2DViewButton, false);
    ...
    myTD.appendChild(selectVue2D);
    myTR.appendChild(myTD);
    myTD = document.createElement('td');
    myTD.appendChild(viewForm);
    myTR.appendChild(myTD);
    center.insertBefore(myTab, center.firstChild);
    insertBr(center.childNodes[1]);
}

function appendBouton(paren, url, id, value, text) {
    ...
    appendSubmit(myForm, text, function () {
...
function putLieuxBouton() {
    ...
    mainTabs[mainTabs.length - 4], null, 9, null).singleNodeValue;
    if (!tdTitle) {
        return;
    }
...
function putSearchForms() {
    ...
    function () {
        filtreMonstre = document.getElementById('rec_monstre').value.toLowerCase();
        filtreMonstres();
    });
    appendSearch(td, 'rec_troll', 'Trõll',
    function () {
        filtreTroll = document.getElementById('rec_troll').value.toLowerCase();
        filtreTrolls();
    });
    appendSearch(td, 'rec_guilde', 'Guilde',
    function () {
        filtreGuilde = document.getElementById('rec_guilde').value.toLowerCase();
        filtreTrolls();
    });
    appendSearch(td, 'rec_tresor', 'Trésor',
    function () {
        filtreTresor = document.getElementById('rec_tresor').value.toLowerCase();
        filtreTresors();
    });
    appendSearch(td, 'rec_lieu', 'Lieu',
    function () {
        filtreLieu = document.getElementById('rec_lieu').value.toLowerCase();
        filtreLieux();
    });
...
function getPosition() {
    ...
    if (posn.indexOf('[') > 0) posn = posn.substr(0, posn.indexOf('[') - 1);
    return new Array(posx, posy, posn);
}
...
try
{
    ...
    putFiltresBoutons();
    putSearchForms();
    putExternalLinks(); // inutilisé ? cf Options
    set2DViewSystem();
    putLieuxBouton();
...
function createBarrePV(color, pvr, pv, comment) { //color: 0=red, 1=gris
    if (pvr > pv) pvr = pv;
    var size = Math.floor((50 * pvr) / pv);
    if ((size < 50) && (size > 48)) size = 48; // pour rendre plus joli
    if (comment == '') var text = pvr + '/' + pv + ' PV';
    else var text = comment;

    if (color == 0) var imgG = '/skin/red.gif';
    else if (color == 1) var imgG = '/skin/grey.gif';
    else imgG = '/skin/white.gif';
    if (color == -3) var imgD = '/skin/green.gif';
    else var imgD = '/skin/white.gif';
...
function createTablePV(color, pvr, pv, comment) { //color: 0=red, 1=gris
    ...
    var myTd = document.createElement('td');
    if (color == 0) myTd.setAttribute('bgcolor', '#FF0000');
    else if (color == 1) myTd.setAttribute('bgcolor', '#AAAAAA');
    else myTd.setAttribute('bgcolor', '#FFFFFF');
    ...
    if (size < 50) {
        ...
        if (color == -3) myTd2.setAttribute('bgcolor', '#00FF00');
        else myTd2.setAttribute('bgcolor', '#FFFFFF');
...
function createMsgPXBouton() {
    ...
    appendSubmit(myForm, 'Envoyer un MP', function () {
        document.getElementsByName('sendMP')[0].action = sendMessagePrive(3);
    });
    myForm.appendChild(document.createTextNode(" "));
    appendSubmit(myForm, 'Partage PX', function () {
        document.getElementsByName('sendMP')[0].action = sendMessagePrive(8);
    });
    myForm.appendChild(document.createTextNode(" "));
    appendSubmit(myForm, 'Partage ZZ', function () {
        document.getElementsByName('sendMP')[0].action = grantPartageZZ();
    });
    return myForm;
}
...
function grantPartageZZ() {
    ...
    {
        if (x_trolls[1 + i].childNodes[2].firstChild.checked) {
            MP += x_trolls[1 + i].childNodes[1].childNodes[0].nodeValue + ',';
        }
    }
...
function sendMessagePrive(cat) {
    ...
    {
        if (x_trolls[1 + i].childNodes[2].firstChild.checked) {
            if (cat == 8) MP += '%2C' + x_trolls[1 + i].childNodes[1].childNodes[0].nodeValue;
            else MP += '+' + x_trolls[1 + i].childNodes[1].childNodes[0].nodeValue + '%2C';
        }
    }
...
function setInfosTrolls() {
  ...
  //  noeud.insertBefore(newTd, noeud.childNodes[4]);
    ...
    newTd.appendChild(newB);
    noeudtroll.insertBefore(newTd, noeudtroll.childNodes[2]);
    ...
    for (var i = 0	; i < x_trolls.length - 1; i++) {
        ...
        var PosX = x_trolls[1 + i].childNodes[4 + incT].childNodes[0].nodeValue;
...
//                    x_trolls[1 + i].style.backgroundColor = "#B1E1DD";
        ...
        if (infosTrolls[num]) {
            ...
            var guilde = "";
            try {
                guilde = x_trolls[1 + i].childNodes[5].childNodes[0].childNodes[0].nodeValue;
            } catch (e) {}
        ...
        x_trolls[1 + i].insertBefore(newTd, x_trolls[1 + i].childNodes[2]);
        var cb = document.createElement('INPUT');
        cb.setAttribute('type', 'checkbox');
    ...
    for (var i = 0; i < x_trolls.length-1; i++) {
        ...
        while (zzTroll < nbIT) {
            ...
            while ((key == num) || (infosTroll[9] < 0)) {
                zzTroll++;
                if (zzTroll >= nbIT) break;
                key = infosTrollID[zzTroll];
                infosTroll = infosTrolls[key];
            }
            if (zzTroll >= nbIT) break;
            ...
            if (flagNewTroll == false) {
                ...
                x_trolls[1 + i].parentNode.insertBefore(trvITM, x_trolls[1 + i]); // insertion dans la liste
        ...
        onload: function (responseDetails) {
            try
            {
                ...
                if (tresorInfo[MatosId]) {
                    ...
                    if (tresor[5] != "0") tresor[5] = "Gowap #" + tresor[5] + " de ";
                    else tresor[5] = "";
                    if (tresor[2].indexOf(':') > 0) var type = tresor[2].substr(0, tresor[2].indexOf(':') - 1);
                    else var type = '';
...
function setInfosTresors() {
    ...
    for (var i = 0; i < x_tresors.length - 2; i++) {
        ...
        if (!bookedCase[tx + ',' + ty + ',' + tn]) {
            ...
            if (ZPOSN == tn) {
                if (isStyleClass) x_tresors[2 + i].setAttribute('class', 'mh_tresors_teleks');
                else x_tresors[2 + i].style.backgroundColor = '#EDE080'; //'#D1CE8C';
            } else {
                if (isStyleClass) x_tresors[2 + i].setAttribute('class', 'mh_tresors_gogos');
                else x_tresors[2 + i].style.backgroundColor = '#EFE7A7'; //'#FFFDAA';
            }
        }
        ...
        if (idt > 0) {
            ...
            if (portailInfo[idt]) {
                ...
                if (portail[3] == "aujourd'hui") flagNew = true;
                    ...
                    if (isStyleClass) x_lieux[2 + i].setAttribute('class', 'mh_pieges_vieux');
                    else x_lieux[2 + i].style.backgroundColor = '#9DA8C6';
                }
            } else {
            ...
            if (flagNew) {
                x_lieux[2 + i].setAttribute('class', '');
                if (isStyleClass) x_lieux[2 + i].setAttribute('class', 'mh_pieges_actifs');
                else x_lieux[2 + i].style.backgroundColor = '#FF5E28';
            }
        }
    }
}
...
function analyseCDM(idx) {
    ...
    if (idMonstre >= 0) var photo = SkinZZ + '/monstres/' + skinBEAST[tabMonstres[idMonstre][3]];
    else photo = SkinZZ + '/monstres/' + skinBEAST[0];
    ...
    else if (cmdMonstre[12] == 0) vlc = "non";
    else vlc = "";
    if (cmdMonstre[13] == 1) attdist = "oui";
    else if (cmdMonstre[13] == 0) attdist = "non";
    else attdist = "";
    ...
    if (cmdMonstre[23]) {
        if (cmdMonstre[23] == 1) vole = "oui";
        else if (cmdMonstre[23] == 0) vole = "non";
        else vole = "";
    }
    if (cmdMonstre[24]) SF = bbcode(cmdMonstre[24]);
    if (cmdMonstre[25]) {
        if (cmdMonstre[25] == 1) attmag = "oui";
        else if (cmdMonstre[25] == 0) attmag = "non";
        else attmag = "";
    }
    ...
    if (cmdMonstre[11].indexOf("%") > 0) strCDM += barrePV.innerHTML + " " + Blessure;
    strCDM += '</TD></TR>';
    ...
    for (var i = 0; i < x_monstres.length-1 ; i++) {
        ...
        if (gogo[num]) { // Affichage des info de Gowap (les gowap ne sont pas insultés
            ...
            if (gogo[num][0] == NUM_TROLL) { // Dilpo des Gogos
                ...
                if (isStyleClass) x_monstres[1 + i].setAttribute('class', 'mh_mes_gogos');
                else x_monstres[1 + i].style.backgroundColor = '#AAFFAA';
            } else {
                x_monstres[1 + i].setAttribute('class', '');
                if (isStyleClass) x_monstres[1 + i].setAttribute('class', 'mh_gogos_coterie');
                else x_monstres[1 + i].style.backgroundColor = '#BBBBFF';
            }
        ...
        if (fam[num]) {
            ...
            if (fam[num][0] == NUM_TROLL) {
                x_monstres[1 + i].setAttribute('class', '');
                if (isStyleClass) x_monstres[1 + i].setAttribute('class', 'mh_mes_fams');
                else x_monstres[1 + i].style.backgroundColor = '#99B599';
            } else {
                x_monstres[1 + i].setAttribute('class', '');
                if (isStyleClass) x_monstres[1 + i].setAttribute('class', 'mh_fams_coterie');
                else x_monstres[1 + i].style.backgroundColor = '#9C9CAF';
            }
        }
        ...
        if (listeCDM[num]) { // On a une les infos d'un bestiaire!
                ...
                if ((distance == 0) || !infosTroll || ((!infosTroll[9]) || (infosMonstre[14] == 'Oui'))) {
                    ...
                    if (vueArr) {
                        if (eval(vueArr[1]) >= distance) {
                            ...
                            x_monstres[1 + i].setAttribute('class', '');
                            if (isStyleClass) x_monstres[1 + i].setAttribute('class', 'mh_vue_monstre1');
                            else x_monstres[1 + i].style.backgroundColor = '#FFBBBB'; //#FF9999
                        }
                        if ((minmax == 1) && (eval(vueArr[1]) < distance && eval(vueArr[2]) >= distance)) {
                            ...
                            x_monstres[1 + i].setAttribute('class', '');
                            if (isStyleClass) x_monstres[1 + i].setAttribute('class', 'mh_vue_monstre2');
                            else x_monstres[1 + i].style.backgroundColor = '#FFD3D3'; //#FFBBBB
                        }
                    }
                }
            } // Fin codage netWorms
            ...
            if (blessure.indexOf('%') < 0) {
                ...
                else if (((cDate - oDate) / 1000 / 60 / 60 / 24) < 5) var myTable = createBarrePV(0, pv, 100, tpv);
                else var myTable = createBarrePV(1, pv, 100, tpv);
...
function putInfosVUE() {
    ...
    if ((setInfosVue()) && (ShrVUE)) sendVue(0);
    else putSendVueBouton(); // // Gestionnaire de vue!!! ou Mettre un bouton pour forcer l'envoye de la vue 
}
...
var tabletroll = document.getElementsByName('trolls')[0].parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode;
putMsgPXBouton(tabletroll); //totaltab[8]); 	// les bouton de distribution de PX, MP et partage
...
//console.log(ZZDB + "/mzVue.php?&TimeStamp=" + TimeStamp + "&num=" + numTroll + "&X=" + ZPOSX + "&Y=" + ZPOSY + "&N=" + ZPOSN + idMonstres + ZPREF);
