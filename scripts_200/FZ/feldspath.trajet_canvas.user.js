...
else if((window.self.location.toString().indexOf("/mountyhall/MH_Lieux/Lieu_Description.php") != -1)) {
// else if(true) { 	
	page = "lieu";
...
function bulleInfosTrolls(desc, xcase, ycase) {
	if (infosTrolls.length==0) return;

	for (var key in infosTrolls) {
     	var infosTroll=infosTrolls[key]; 

		if  ((xcase==parseInt(infosTroll[6])) && (ycase==parseInt(infosTroll[7]))) {		// ZZ: ajouter l'affichage des copains!
			desc.appendChild(document.createTextNode(infosTroll[5]+": n= "+infosTroll[8]));
			desc.appendChild(document.createElement("br"));
		}
...
function putInfosMAP() {

	if (infosTrolls.length==0) return;
	dessin = document.getElementById("trou");
	if (dessin.getContext){
		var ctx = dessin.getContext('2d');
		ctx.strokeStyle = "rgba(51,51,255,0.6)";
		ctx.lineWidth = 2;
		ctx.beginPath();

  		for (var key in infosTrolls) if (key != numTroll){
     		var infosTroll=infosTrolls[key]; 
     		var posX=parseInt(infosTroll[6]); 
     		var posY=parseInt(infosTroll[7]); 
	 		ctx.moveTo(coord_x(posX)-4, coord_y(posY)-4);
			ctx.lineTo(coord_x(posX)+4, coord_y(posY)+4);
			ctx.moveTo(coord_x(posX)+4, coord_y(posY)-4);
			ctx.lineTo(coord_x(posX)-4, coord_y(posY)+4);
		}
		ctx.stroke();

		// ajout du troll lui même!
  		if (infosTrolls[numTroll]){
			var ctx = dessin.getContext('2d');
			ctx.strokeStyle = "rgba(0,0,0,1)";
			ctx.lineWidth = 2;
			ctx.beginPath();
     		var infosTroll=infosTrolls[numTroll]; 
     		var posX=parseInt(infosTroll[6]); 
     		var posY=parseInt(infosTroll[7]); 
	 		ctx.moveTo(coord_x(posX)-4, coord_y(posY)-4);
			ctx.lineTo(coord_x(posX)+4, coord_y(posY)+4);
...
if (page=="lieu") 	ini_empty();	

//alert(ZZDB+"/mzMap.php?&num="+numTroll);
if (page) MZ_xmlhttpRequest({
				method: 'GET',
				url: ZZDB+"/mzMap.php?&num="+numTroll,	
				headers : {
